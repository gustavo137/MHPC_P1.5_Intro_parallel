...
void plot_data_1d(char *name, int n1, int n2, int n3, int dir, double *data) {
  ...
  /*
   * HINT: Assuming you sliced your system along i3, iif idir.eq.1 or idir.eq.2
   * you need to understand which process holds the line you are inrested in. If
   * idir.eq.3 you need to take the correct slice of the line from each process
   */
...
void plot_data_2d(char *name, int n1, int n2, int n3, int dir, double *data) {
   ...
   *       need to take the correct slice of the plane from each process. If
   * idir==3, you need to understand which process holds the plane you are
   * inrested in.
   */
  if (dir == 1) {
    i1 = n1 / 2 - 1;
...
void plot_data_1d_parallel(char *name, int n1, int n2, int n3, int n1_local,
  ...
  if (dir == 1) {
    ...
    MPI_Gatherv(&data[index], n1_local, MPI_DOUBLE, buffer, sizes, displ,
...
void plot_data_2d_parallel(char *name, int n1, int n2, int n3, int n1_local,
                           int n1_local_offset, int dir, double *data) {
  int i1, i2, i3, i;
  FILE *fp;
  int num = 1;
  ...
  if (dir == 1) {
    ...
    for (i3 = 0; i3 < n3; ++i3) {
      ...
      MPI_Gatherv(local_buffer, n1_local, MPI_DOUBLE, buffer1d, sizes, displ,
                  MPI_DOUBLE, 0, MPI_COMM_WORLD);
      for (i1 = 0; i1 < n1; ++i1) {
