...
int main(int argc, char *argv[]) {
  ...
  */
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),(int)(fft_h.local_n1_offset), 1, diffusivity);
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),(int)(fft_h.local_n1_offset), 2, diffusivity);
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),(int)(fft_h.local_n1_offset), 3, diffusivity);
  ...
  for (istep = 1; istep <= nstep; ++istep) {
    ...
    if (istep % 30 == 1) {
      ...
      // HINT: the conc array is distributed, so only a part of it is on each
      // processor
      for (i3 = 0; i3 < n3; ++i3) {
       ...
       *
       */
      MPI_Allreduce(&r2mean, &global_r2mean, 1, MPI_DOUBLE, MPI_SUM,
                    MPI_COMM_WORLD);
      MPI_Allreduce(&ss, &global_ss, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      global_ss *= fac;
      global_r2mean *= fac;
      ...
      printf(" %d %17.15f %17.15f Elapsed time per iteration %f \n ", istep,
               global_r2mean, global_ss, (end - start) / istep);
      ...
      */
      ...
      plot_data_2d("concentration", n1, n2, n3, (int)(fft_h.local_n1),
                   (int)(fft_h.local_n1_offset), 2, conc);
      plot_data_1d("1d_conc", n1, n2, n3, (int)(fft_h.local_n1),
                   (int)(fft_h.local_n1_offset), 3, conc);
    }
  }
