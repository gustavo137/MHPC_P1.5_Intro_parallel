...
void init_fftw(fftw_mpi_handler *fft, int n1, int n2, int n3, MPI_Comm mpi_comm) {
  ...
  // Allocate the fftw_data array
  fft->fftw_data = fftw_malloc(sizeof(fftw_complex) * local_size_grid);
  if (fft->fftw_data == NULL) {
    fprintf(stderr, "Error allocating memory for FFTW data.\n");
    MPI_Abort(fft->mpi_comm, -1);
  }
  
  // Create the FFTW plans
  fft->fw_plan = fftw_mpi_plan_dft_3d(n1, n2, n3, fft->fftw_data, fft->fftw_data,
                                      mpi_comm, FFTW_FORWARD, FFTW_MEASURE);
  fft->bw_plan = fftw_mpi_plan_dft_3d(n1, n2, n3, fft->fftw_data, fft->fftw_data,
                                      mpi_comm, FFTW_BACKWARD, FFTW_MEASURE);
}
...
void fft_3d(fftw_mpi_handler* fft, int n1, int n2, int n3,
            double *data_direct, fftw_complex* data_rec, bool direct_to_reciprocal) {

    ptrdiff_t i;
    if (direct_to_reciprocal) {
        // Allocate a complex array for input
...
/*
void init_fftw_old(fftw_mpi_handler *fft, int n1, int n2, int n3, MPI_Comm mpi_comm) {
  // fftw_mpi_init();  // Removed redundant call since it's called in main
  fft->mpi_comm = mpi_comm;

  // Adjusting variable names to match the struct definition
  ptrdiff_t global_size_grid, local_size_grid, local_n1, local_n1_offset;
  global_size_grid = n1 * n2 * n3;
  fft->global_size_grid = global_size_grid;
...
/*
void fft_3d_old(fftw_mpi_handler* fft, int n1, int n2, int n3, double *data_direct, fftw_complex* data_rec, bool direct_to_reciprocal){
  double fac;
  int i;
