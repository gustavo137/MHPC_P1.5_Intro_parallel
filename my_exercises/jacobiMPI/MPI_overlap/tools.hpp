...
void CSolver<U>::jacobi(CMesh<U> &M, const size_t &ite, const size_t &pI) {
  ...
  for (int it = 0; it <= ite; it++) {
   ...
   // {Parallel_Timer t("Comunication_Time");
    ...
    // THe mean function is this one, I put evolve inside:
...
// evolve ends
//////// function exchange_ghost_cells(Mesh<T>& m); Sendrecv non bloking 
/*
template <typename T>
...
void CSolver<T>::exchange_ghost_cells(CMesh<T>& M) {
    ...
    // Buffers for non-blocking send and receive
    std::vector<T> send_to_up(dim + 2);
    std::vector<T> recv_from_down(dim + 2);
    std::vector<T> send_to_down(dim + 2);
    std::vector<T> recv_from_up(dim + 2);
    ...
    MPI_Irecv(recv_from_up.data(), dim + 2, MPI_DOUBLE, up_rank, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Irecv(recv_from_down.data(), dim + 2, MPI_DOUBLE, down_rank, 1, MPI_COMM_WORLD, &requests[1]);
    MPI_Isend(send_to_up.data(), dim + 2, MPI_DOUBLE, up_rank, 1, MPI_COMM_WORLD, &requests[2]);
    MPI_Isend(send_to_down.data(), dim + 2, MPI_DOUBLE, down_rank, 0, MPI_COMM_WORLD, &requests[3]);
  
 
  {Parallel_Timer t("Calculation_Time:");
    // Compute the interior cells while communication is in progress
    evolve(M.matrix, M.new_matrix, dim_local, dim);
  }
